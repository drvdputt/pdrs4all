...
# JJ is the number of processes for stage 3. The required memory depends on the final shape of
# the mosaic.
JJ=1
...
parallel_shorthand () {
    ...
    echo parallel --retries 3 --joblog $2.joblog --progress -j $1 {} ">>"$OUT_LOG/$2_cpu{%}.log '2>&1' :::: jobs_$2.sh
    parallel --retries 3 --resume --joblog $2.joblog --progress -j $1 {} ">>"$OUT_LOG/$2_cpu{%}.log '2>&1' :::: jobs_$2.sh
}
...
pipeline_jobs -s 2 -b $OUT_BKG -o $OUT_SCI $IN_SCI
mv strun_calwebb_image2_jobs.sh jobs_sci_2.sh
parallel_shorthand $J sci_2

pipeline_jobs -s 3 --mosaic -o $OUT_SCI $IN_SCI
mv strun_calwebb_image3_jobs.sh jobs_sci_3.sh
