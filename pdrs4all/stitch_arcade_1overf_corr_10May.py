...
output_dir_reprojected_s3d = '/arc/projects/PDRs4All/NIRSpec_1_f_corr/stitching/reprojected_s3d/'
output_dir_stitched_cubes = '/arc/projects/PDRs4All/NIRSpec_1_f_corr/stitching/stitched_cubes/all_segments/'
...
# create a Stitcher
stest = stitch.Stitcher(fnames_s3d_f100lp=fnames_f100lp,
...
def load_cal_dict(version_tag_xcal):
    ...
    if version_tag_xcal == 'corner_only_mask_9June23':
        ...
        fnames_xcal_f100lp = []
        fnames_xcal_f170lp = [fname_xcal_1overf(version_tag_xcal, 'f182m'), fname_xcal_1overf(version_tag_xcal, 'f210m')]
        fnames_xcal_f290lp = [fname_xcal_1overf(version_tag_xcal, 'f405n'), fname_xcal_1overf(version_tag_xcal, 'f335m')]

    cal_dict = xcal.make_cal_dict(fnames_xcal_f100lp, fnames_xcal_f170lp,
                                fnames_xcal_f290lp)
    print(cal_dict)
...
def plot_all_xcal_results(version_tag_xcal, plot_dir):
    ...
    'CLEAR-F187N',
    'F405N-F444W']
    filters = [fc.split('-')[-1] if 'CLEAR' in fc else fc.split('-')[0] for fc in filt_combos]
...
if __name__ == '__main__':
    ...
    if "--corners_only" in sys.argv:
        ...
        version_tag_xcal = 'corner_only_mask_9June23' # non-background subtracted nircam
    ...
    if fun == 'step0_mkdir':
        ...
        os.system(
            'mkdir -p /arc/projects/PDRs4All/NIRSpec_1_f_corr/synthetic_images/'
        )
        os.system(
    ...
    if fun == 'step3_coadd_full_mosaic':
        ...
        with open(
                f'/arc/projects/PDRs4All/NIRSpec_1_f_corr/stitching/test_coadd_masks_8June23/coadd_masks_topleft_bottomright.pkl',
                'wb') as fl:
            pickle.dump(coadd_masks,
                        fl,
                        compression=None,
                        set_default_extension=False)
    ...
    if fun == 'step3_realign_full_mosaic':
        stest.realign_nirspec_fits('F212N',
                                   fname_synth_images_fits,
                                   fname_synth_images_unc_fits,
                                   calibrated=False,
                                   scaled=False)
    ...
    if fun == 'step3_coadd_single_seg_mosaics_cal':
        ...
        # for seg in ['f100lp', 'f170lp', 'f290lp']:
        plist = ['Pointing_' + p for p in plist]
        cal_dict, fnames_xcal_f100lp, fnames_xcal_f170lp, fnames_xcal_f290lp = load_cal_dict(version_tag_xcal)
        fname_calfac_lists = [
    ...
    if fun == 'step4_prepare_nircam_images':
        for fname_nircam in fnames_nircam_bkgsub:
            stest.reproject_nircam(fname_nircam,
                                   fname_nircam_bkgsub_reproj(fname_nircam))
    ...
    if fun == 'step6_xcal_all_ptg_not_cal_not_scaled':
        ...
        if filt == 'all':
            ...
            filters = sorted([fc.split('-')[-1] if 'CLEAR' in fc else fc.split('-')[0] for fc in filt_combos])[:3]
        else:
            filters = [filt]
        ...
        for ftemp in filters:
            ...
            fname = search_list_of_filenames_for_pattern(
                fnames_nircam_bkgsub, look_for)
