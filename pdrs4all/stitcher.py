...
class Stitcher:
    ...
    def __init__(self,
        ...
        self.fname_wcs_corrected = f'{self.output_dir_wcs}optimal_wcs_nirspec_mosaic_corrected.pkl'
        ...
        self.fname_stitched_cube_all_pointings_wcs_corrected_calibrated = fname_fits_add_wcscorr(
            self.fname_stitched_cube_all_pointings_calibrated)
        self.fname_mask_stitched_cube_all_pointings = lambda edge_dist_thresh_pix: f"{self.output_dir_stitched_cubes}minimal_edge_mask_{edge_dist_thresh_pix}pix.fits.gz"
    ...
    def fname_reprojected_s3d(self, fname_cube, uncertainty):
        ...
        if uncertainty:
            unc_str = '_unc'
        fname = self.output_dir_reprojected_s3d + fname_cube.split(
            '/')[-1].split('.')[0] + unc_str + '.pk.gz'
        return fname
    ...
    def fnames_s3d_with_pattern(self, patterns):
        ...
        '''
        fnames_s3d_all_pointings = self.fnames_s3d_f100lp + self.fnames_s3d_f170lp + self.fnames_s3d_f290lp
        res = [
            fname for fname in fnames_s3d_all_pointings
    ...
    def fname_stitched_cube_single_pointing(self,
                                            pointing_label,
                                            calibrated=False,
                                            scaled=True):
        cal_str = ''
    ...
    def fname_mosaic_single_segment(self,
                                    seg,
                                    wcscorr=True,
                                    calibrated=False,
                                    scaled=True):
    ...
    def calculate_optimal_wcs_for_nirspec_mosaic(self, save=True):
        ...
        ra, dec, w = stitch_utils.get_ra_dec_grid_optimal_wcs(
    ...
    def reproject_cube(self, fname_cube, uncertainty, clip_spikes=True):
        ...
        cube = stitch_utils.projection_field(fname_cube,
                                             w,
                                             shape_out,
                                             uncertainty=uncertainty,
                                             clip_spikes=clip_spikes)
        # Save
        res = dict()
        res['cube'] = cube
    ...
    def reproject_set_of_cubes(self,
                               list_of_fnames,
                               uncertainty,
                               clip_spikes=True):
        '''
        ...
        for fname_cube in list_of_fnames:
            self.reproject_cube(fname_cube,
                                uncertainty,
                                clip_spikes=clip_spikes)

    def reproject_set_of_cubes_with_pattern(self,
                                            patterns,
                                            uncertainty,
                                            clip_spikes=True):
        '''
    ...
    def run_ers1288_nirspec(self,
        ...
        list_of_fnames = self.fnames_s3d_with_pattern(
            [pointing_label, '100lp']) + self.fnames_s3d_with_pattern([
                pointing_label, '170lp'
        ...
        cube = pickle.load(open(fname_cube_reproj, 'rb'),
                           compression='gzip')['cube']

        # Uncertainty
        fname_cube_unc_reproj = self.fname_reprojected_s3d(fname_cube, True)
        cube_unc = pickle.load(open(fname_cube_unc_reproj, 'rb'),
                               compression='gzip')['cube']

        # Apply calibration factor
        def apply_calfac(cube, cube_unc, calfac, calfac_err):
            cube_unc = np.abs(
                cube * calfac) * np.sqrt((cube_unc / cube)**2 +
                                         (calfac_err * calfac)**2)
            cube = cube * calfac
            return cube, cube_unc

        i_cal = 0
        if (cal_dict[cal_keys[i_cal]]['calfac'] == 1.
            ):  # and (do_not_scale == False):
            i_cal = 1
            print(
                f"First cal factor is 1.0. Applying cal factor for 2nd seg instead ({cal_dict[cal_keys[1]]['calfac']})"
            )

        cube, cube_unc = apply_calfac(cube, cube_unc,
        ...
        for i in range(1, len(list_of_fnames)):
            ...
            fname_cube_reproj = self.fname_reprojected_s3d(fname_cube, False)
            cube_i = pickle.load(open(fname_cube_reproj, 'rb'),
                                 compression='gzip')['cube']

            fname_cube_unc_reproj = self.fname_reprojected_s3d(
                fname_cube, True)
            cube_unc_i = pickle.load(open(fname_cube_unc_reproj, 'rb'),
                                     compression='gzip')['cube']

            cube_i, cube_unc_i = apply_calfac(
                cube_i, cube_unc_i, cal_dict[cal_keys[i]]['calfac'],
        ...
        if do_not_scale:
            ...
            for x in range(0, nx):
                for y in range(ny):
                    result = 0
                    if cube1_mask[x, y] + cube2_mask[x, y] + cube3_mask[x,
                                                                        y] > 0:
                        print('x, y = ', x, y)
            ...
            for x in range(nx):
                for y in range(ny):
                    ...
                    print('x, y = ', x, y)
                    scale_factor_seg2_seg3 = 1.
                    scale_factor_seg1_seg2 = 1.
                    if cube1_mask[x, y] + cube2_mask[x, y] + cube3_mask[x,
                                                                        y] > 0:
    ...
    def stitch_and_save_single_pointing(self,
                                        pointing_label,
                                        cal_dict=None,
                                        snr_cutoff=None,
                                        force_zero_scale_factor=False,
    ...
    def coadd_pointings(self,
        ...
        if seg == 'all':
            ...
            if wcscorr:
                ...
                if calibrated:
                    fname_out = self.fname_stitched_cube_all_pointings_wcs_corrected_calibrated
            else:
            ...
            fname_out = self.fname_mosaic_single_segment(seg,
                                                         wcscorr=wcscorr,
        ...
        print(
            "Calculating coverage array and distance transform for each pointing..."
        )
        ...
        for i in range(n_pointings - 1):
            ...
            if mask_top_left_bottom_right:
                ...
                both_covered = (coverage_mask[i] == 1) & (coverage_mask[i + 1]
                                                          == 1)
                ...
                idcs_worst_case_fix = not_already_masked & both_covered & both_close_to_edge & (
                    ys >= ymin_i) & (ys <= ymin_i + edge_dist_thresh_pix) & (
                        xs <= xmax_i) & (xs >= xmax_i - edge_dist_thresh_pix)
                ...
                idcs_worst_case_fix_lower = both_covered & (
                    distance_transforms[i] <= edge_dist_thresh_pix
                ) & (distance_transforms[i + 1] <= edge_dist_thresh_pix) & (
                    xs >= xmin_i) & (xs <= xmin_i + edge_dist_thresh_pix) & (
                        ys <= ymax_i) & (ys >= ymax_i - edge_dist_thresh_pix)
                ...
                coadd_mask_dict[f'iloop{i}'][f'stage_{stage}']['i+1'] = coadd_mask[i+1].copy()
        ...
        if make_save_minimal_edge_mask:
            ...
            sum_coverage = np.sum(coverage_mask, axis=0)
            sum_coadd = np.sum(coadd_mask, axis=0)
        ...
        for i, pointing_label in enumerate(pointing_labels):
            if seg == 'all':
                ...
                with fits.open(fname_cube) as f:
                    ...
                    good = (wt_i != 0) & np.isfinite(wt_i) & (
                        cube_i != 0) & np.isfinite(cube_i)
                    wt_i[good] = 1. / wt_i[good]**2
                ...
                # Load the first cube
                fname_cube = list_of_fnames[0]
                fname_cube_reproj = self.fname_reprojected_s3d(
                    fname_cube, False)
                cube = pickle.load(open(fname_cube_reproj, 'rb'),
                                   compression='gzip')['cube']
                cube = reshape_cube(cube)
        ...
        comb_cube_unc[good] = 1. / np.sqrt(sum_weights[good])
    ...
    def make_and_save_synthetic_images(self, fname_cube,
    ...
    def make_and_save_synthetic_images_mosaic(self,
                                              fname_synth_images_fits,
                                              fname_synth_images_unc_fits,
                                              path_to_throughputs,
                                              seg='all',
        ...
        return self.make_and_save_synthetic_images(
            fname_cube, fname_synth_images_fits, fname_synth_images_unc_fits,
            path_to_throughputs)

    # Calculate wcs offsets
    def realign_nirspec_fits(self,
                             filt,
                             fname_synth_images_fits,
                             fname_synth_images_unc_fits,
                             search_gaia=False,
        ...
        if calibrated:
            fname_out_cube = self.fname_stitched_cube_all_pointings_wcs_corrected_calibrated
        if scaled:
        ...
        if scaled:
            fname_out_synth_unc = fname_out_synth_unc.replace(
                '.fits', '_scaled.fits')

        fname_out_wcs = self.fname_wcs_corrected
        wcs_offsets.realign_nirspec_fits(filt,
                                         fname_synth_images_fits,
                                         fname_synth_images_unc_fits,
                                         fname_cube,
                                         fname_out_cube,
    ...
    def reproject_nircam(self, fname_nircam, fname_out):
        ...
        stitch_utils.reproject_nircam(fname_nircam, wcs_out, shape_out,
                                      fname_out)

    def coadd_pointings_one_segment(self,
                                    segment_label='',
                                    edge_dist_thresh_pix=6,
                                    arcade=True,
                                    outlier_off=False,
        ...
        if which_input_dir == '':
            fname_out = f'{output_dir}Level3_{tag1}_all_pointings_{segment_label}_s3d.fits.gz'
        ...
        for i in range(n_pointings - 1):
            ...
            # overlap = (coverage_mask[i] == 1) & (coverage_mask[i + 1] == 1)
            # Worst case: both close to border
            idcs_worst_case = (coverage_mask[i] == 1) & (
                coverage_mask[i + 1]
                == 1) & (distance_transforms[i] <= edge_dist_thresh_pix) & (
                    distance_transforms[i + 1] <= edge_dist_thresh_pix)
            # Favour i + 1
            coadd_mask[i][idcs_worst_case] = 0
            coadd_mask[i + 1][idcs_worst_case] = 1
            ...
            ymin_i = np.min(ys[idcs_worst_case])
            xmax_i = np.max(xs[idcs_worst_case])
            idcs_worst_case_fix = (coverage_mask[i] == 1) & (
                coverage_mask[i + 1]
                == 1) & (distance_transforms[i] <= edge_dist_thresh_pix) & (
                    distance_transforms[i + 1] <= edge_dist_thresh_pix
                ) & (ys >= ymin_i) & (ys <= ymin_i + edge_dist_thresh_pix) & (
            ...
            xmin_i = np.min(xs[idcs_worst_case])
            ymax_i = np.max(ys[idcs_worst_case])
            idcs_worst_case_fix_lower = (coverage_mask[i] == 1) & (
                coverage_mask[i + 1]
                == 1) & (distance_transforms[i] <= edge_dist_thresh_pix) & (
                    distance_transforms[i + 1] <= edge_dist_thresh_pix
                ) & (xs >= xmin_i) & (xs <= xmin_i + edge_dist_thresh_pix) & (
        ...
        # ]
        unc_str = ''
        name_cube_pickle = files3d[0].split('/')[-1].split(
            '.')[0] + unc_str + '.pk.gz'
        cube = pickle.load(
            open(path_to_stitched_cube_data + name_cube_pickle, 'rb'))['cube']
        nx, ny, nw = cube.shape
        comb_cube = np.zeros((nw, nx, ny))
        comb_cube_unc = np.zeros((nw, nx, ny))
        ...
        for i, pointing_label in enumerate(pointing_labels):
            ...
            if which_input_dir == '':
                ...
                print(f"    {fname_cube}")
                unc_str = ''
                name_cube_pickle = fname_cube.split('/')[-1].split(
                    '.')[0] + unc_str + '.pk'
                unc_str = '_unc'
                name_cube_unc_pickle = fname_cube.split('/')[-1].split(
                    '.')[0] + unc_str + '.pk'
            else:
            ...
            cube = pickle.load(open(
                path_to_stitched_cube_data + name_cube_pickle, 'rb'),
                               compression='gzip')['cube']

            cube = reshape_cube(cube)
            # Uncertainties
            cube_unc = pickle.load(open(
                path_to_stitched_cube_data + name_cube_unc_pickle, 'rb'),
                                   compression='gzip')['cube']
            ...
            wt_i = cube_unc * coadd_mask[i]
            cube_i = cube * coadd_mask[i]
            good = (wt_i != 0) & np.isfinite(wt_i) & (cube_i !=
                                                      0) & np.isfinite(cube_i)
            wt_i[good] = 1. / wt_i[good]**2
            sum_weights[good] += wt_i[good]
            comb_cube[good] += (wt_i[good] * cube_i[good])
        ...
        print(f"Using reference cube to save wcs-corrected cube:")
    ...
    def measure_xcal(self,
        ...
        """
        import xcal
        fname_mask = self.fname_mask_stitched_cube_all_pointings(
            edge_dist_thresh_pix)
        ...
        if filt == 'F210M':
            xmax_limit = 1000
        res_xc = xcal.measure_cal_from_nircam_nirspec(synth_im,
                                                      synth_im_unc,
                                                      w,
                                                      nircam_im,
                                                      nircam_im_unc,
